...
var = "AvailableE" #"pTmu" #"NewEAvail" #"pTmu"
dataName = var + "_data"
...
mcTotal.SetMaximum(mcTotal.GetMaximum() * 2)
#mcTotal.Draw("E2") #Draw the error envelope only.  Not used here except to force the THStack to calculate its sum.
mcTotal.SetTitle("MC")
mcTotBinNorm = mcTotal.GetBinNormalizedCopy().GetCVHistoWithError().Clone()
mcTotBinNorm.SetLineWidth(lineSize)
mcTotBinNorm.GetXaxis().SetTitle("Available Energy (MeV)")
mcTotBinNorm.SetMaximum(mcTotal.GetMaximum() * 2)
mcTotBinNorm.SetName("MC")
mcTotBinNorm.Draw("h,x0")
for hist in mcStack.GetHists():
...
overall.Print(var + "_GENIE_DataMCRatio.png")

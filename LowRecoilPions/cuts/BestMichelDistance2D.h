  ...
  private:
    ...
    bool checkCut(const UNIVERSE& univ, EVENT& evt) const
    {
      ...
      for (unsigned int i = 0; i < evt.m_nmichels.size(); i++)
      {
        ...
        if (upvtxXZ < m_maxDistance && (upvtxUZ < m_maxDistance || upvtxVZ < m_maxDistance)) evt.m_nmichels[i].passable_matchtype.at(0) = 1;
        else if (upvtxUZ < m_maxDistance && (upvtxXZ < m_maxDistance || upvtxVZ < m_maxDistance)) evt.m_nmichels[i].passable_matchtype.at(0) = 1;
        else if (upvtxVZ < m_maxDistance && (upvtxXZ < m_maxDistance || upvtxUZ < m_maxDistance)) evt.m_nmichels[i].passable_matchtype.at(0) = 1;
        else evt.m_nmichels[i].passable_matchtype.at(0) = -1;

        if (downvtxXZ < m_maxDistance && (downvtxUZ < m_maxDistance || downvtxVZ < m_maxDistance)) evt.m_nmichels[i].passable_matchtype.at(1) = 2;
        else if (downvtxUZ < m_maxDistance && (downvtxXZ < m_maxDistance || downvtxVZ < m_maxDistance)) evt.m_nmichels[i].passable_matchtype.at(1) = 2;
        else if (downvtxVZ < m_maxDistance && (downvtxXZ < m_maxDistance || downvtxUZ < m_maxDistance)) evt.m_nmichels[i].passable_matchtype.at(1) = 2;
        else evt.m_nmichels[i].passable_matchtype.at(1) = -1;
        ...
        if (upclusXZ < m_maxDistance && (upclusUZ < m_maxDistance || upclusVZ < m_maxDistance)) evt.m_nmichels[i].passable_matchtype.at(2) = 3;
        else if (upclusUZ < m_maxDistance && (upclusXZ < m_maxDistance || upclusVZ < m_maxDistance)) evt.m_nmichels[i].passable_matchtype.at(2) = 3;
        else if (upclusVZ < m_maxDistance && (upclusXZ < m_maxDistance || upclusUZ < m_maxDistance)) evt.m_nmichels[i].passable_matchtype.at(2) = 3;
        else evt.m_nmichels[i].passable_matchtype.at(2) = -1;
   
        if (downclusXZ < m_maxDistance && (downclusUZ < m_maxDistance || downclusVZ < m_maxDistance)) evt.m_nmichels[i].passable_matchtype.at(3) = 4;
        else if (downclusUZ < m_maxDistance && (downclusXZ < m_maxDistance || downclusVZ < m_maxDistance)) evt.m_nmichels[i].passable_matchtype.at(3) = 4;
        else if (downclusVZ < m_maxDistance && (downclusXZ < m_maxDistance || downclusUZ < m_maxDistance)) evt.m_nmichels[i].passable_matchtype.at(3) = 4;
        else evt.m_nmichels[i].passable_matchtype.at(3) = -1;

        std::vector<double> distances3D;
        
        if (evt.m_nmichels[i].passable_matchtype[0] == 1) distances3D.push_back(evt.m_nmichels[i].up_to_vertex_dist3D);
        if (evt.m_nmichels[i].passable_matchtype[1] == 2) distances3D.push_back(evt.m_nmichels[i].down_to_vertex_dist3D);
        if (evt.m_nmichels[i].passable_matchtype[2] == 3) distances3D.push_back(evt.m_nmichels[i].up_clus_michel_dist3D);
        if (evt.m_nmichels[i].passable_matchtype[3] == 4) distances3D.push_back(evt.m_nmichels[i].down_clus_michel_dist3D);
        ...
        //std::cout << "Passable Match Types for this Michel are " << evt.m_nmichels[i].passable_matchtype.at(0) << evt.m_nmichels[i].passable_matchtype.at(1) << evt.m_nmichels[i].passable_matchtype.at(2) << evt.m_nmichels[i].passable_matchtype.at(3) << std::endl;
        ...
        //std::cout << "This  Michel is UPVTX and has true endpoint " << evt.m_nmichels[i].trueEndpoint << std::endl;
     	}
         ...
         //std::cout << "This  Michel is DOWNVTX and has true endpoint " << evt.m_nmichels[i].trueEndpoint << std::endl;
        ...
        //std::cout << "This  Michel is UPCLUS and has true endpoint " << evt.m_nmichels[i].trueEndpoint << std::endl;

     	}
     	else if (distances3D[0] == evt.m_nmichels[i].down_clus_michel_dist3D){
        evt.m_nmichels[i].BestMatch = 4;
        evt.m_nmichels[i].Best3Ddist = evt.m_nmichels[i].down_clus_michvtx_dist3D;
        ...
        //std::cout << "This  Michel is DOWNCLUS and has true endpoint " << evt.m_nmichels[i].trueEndpoint << std::endl;

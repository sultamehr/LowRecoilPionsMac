...
class FSCategory
 {
	...
	const int interaction = univ.GetInteractionType();
        int pions = univ.GetTrueNPionsinEvent();
	int kaons = univ.GetTrueNKaonsinEvent();
	int pi0 = univ. GetTrueNPi0inEvent();
	int qelike = 0;
	...
	//else if (interaction==3){return false;}
		...
		if (pions > 0 && fForbidden.count(9999) == 1) return true;
		else if ((kaons > 0) && fRequired.count(321) == 1 ) return true;
		else if (pi0 > 0 && fRequired.count(111) == 1) return true;
		else if (pions == 0 && kaons == 0 && pi0 == 0 && fRequired.count(9999) == 1) return true;
		else return false;
        	//return true;
	}
      }
